...
class Http_API(object):
    ...
    def postRequest(self, url=None, data=None):
        ...
        print("aixo: ", json.dumps(data))
        #json=json.dumps(data)
        response = requests.post(url=url, json=data, headers={"Content-Type":"application/json"}, timeout=10)
...
class ThreadForQT(QThread):
    ...
    def set_ota_flash(self, **info):
        ...
        sub_id = info["sub_id"]
        data["deviceid"] = sub_id
        urldownload = "http://" + info["sever_ip"] + ":" + \
            str(info["sever_port"]) + "/tasmota-lite.bin"

        data["data"] = {"downloadUrl": urldownload,"sha256sum":info["sha256sum"]}
        # 3. Call Http_API(postRequest) to send.
        return self.send_data(send_url=url, send_data=data)
